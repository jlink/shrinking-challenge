...
class Bound5Spec extends AnyFlatSpec with Matchers {
  ...
  "the sum" should "be nor more than five times the limit on the individual sums" in {
    quintuples.withStrategy(caseSupplyCycle => CasesLimitStrategy.timed(Duration.ofSeconds(1 + caseSupplyCycle.numberOfPreviousShrinkages())), shrinkageStop = snoopOnShrinkage).supplyTo { quintuple =>
